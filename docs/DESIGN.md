...
画像全体のカメラ移動と、選択範囲の部分モーションは別系統として表示する。`textLock` が有効な間はカメラ選択欄を無効化し、停止理由を画面へ表示する。部分モーションの `axis` は、`sway` と `drift` で左右・上下・斜めを選べる。

編集履歴はプロジェクトのスナップショットを最大50件までメモリに保持する。画像・音声ファイル本体は履歴JSONへ複製せず、シーンIDごとの実行時参照を引き継ぐ。

最初の実装では背景の自動インペイントを必須にしない。マスク周囲の画素を内側へ伸ばしてぼかす簡易背景補完で、小さな揺れの二重写りを抑える。複雑な背景向けには、将来手動背景画像と自動インペイントを選べるようにする。
